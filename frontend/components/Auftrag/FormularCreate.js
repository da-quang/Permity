...
function FormularCreate(props) {
  ...
  const CREATE2 = async () => {
    const response = await fetch(
      `https://palmiest-hornet-1388.dataplicity.io/api/api/Auftrag/create?ksv=${props.Ksv}&auftrag=${props.Auftrag}&auftraggeber=${props.Auftraggeber}&auftragnehmer=${props.Auftragnehmer}&sperren=${props.Sperren}&kommentar=${props.Kommentar}&von=${props.Von}&bis=${props.Bis}&auftraggeber_unterschrift=${props.AuftraggeberUnterschrift}`,
      {
        method: "POST",
      }
    );
    ...
    <div>
      <ThemeProvider theme={BTNTheme}>
        <Button
          ...
          size="large"
          disabled={props.AuftraggeberUnterschrift != "null" ? false : true}
          variant="contained"
          onClick={() => {
            CREATE2();
            MAIL();
